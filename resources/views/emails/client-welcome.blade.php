...
<body>
    <div class="container">
        <div class="header">
            <h1>⚖️ {{ $firmName }}</h1>
            <p>{{ __('البوابة الإلكترونية للموكلين') }}</p>
        </div>
        <div class="body">
            <h2>مرحباً {{ $clientName }}،</h2>
            <p>
                {{ __('نود إعلامك بأنه تم إنشاء حساب لك في البوابة الإلكترونية لـ') }} <strong>{{ $firmName }}</strong>{{ __('.
                من خلال هذه البوابة، يمكنك متابعة تقدم قضاياك، مواعيد الجلسات، ورصيد الأتعاب.') }}
            </p>
            <p>{{ __('لتفعيل حسابك وتعيين كلمة مرور خاصة بك، يرجى الضغط على الزر أدناه:') }}</p>
            <div style="text-align:center;">
                <a href="{{ $activationUrl }}" class="btn">{{ __('🔑 تفعيل الحساب وتعيين كلمة المرور') }}</a>
            </div>
            <div class="notice">
                {{ __('⏱️ رابط التفعيل صالح لمدة') }} <strong>{{ __('48 ساعة') }}</strong> {{ __('من تاريخ إرسال هذا البريد. إذا لم تطلب هذا الحساب، يمكنك تجاهل هذه الرسالة.') }}
            </div>
        </div>
        <div class="footer">
