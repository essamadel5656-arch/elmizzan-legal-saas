...
<div class="create-page-container">
    ...
    <div class="dashboard-header no-print">
        <div class="header-info">
            <h1><i class="fas fa-home" style="color: var(--gold-accent); margin-left: 8px;"></i> {{ __('عقد بيع حصة شائعة في منزل') }}</h1>
            <p>{{ __('أدخل بيانات أطراف العقد وتفاصيل الحصة الشائعة ثم اضغط على زر المعاينة والطباعة أدناه.') }}</p>
        </div>
    ...
    <div class="no-print">
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="contract_day" class="form-control" placeholder="{{ __('مثال: الأربعاء') }}">
                </div>
                <div class="form-group">
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label class="form-label" for="seller_job">{{ __('الوظيفة / المهنة') }}</label>
                    <input type="text" id="seller_job" class="form-control" placeholder="{{ __('مثال: مأمور ضرائب - بالمعاش') }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group full" style="border-top: 1px dashed var(--border-color); padding-top: 1rem; margin-top: 0.5rem;">
                    <label class="form-label" for="inheritance_from" style="color: var(--sidebar-bg);">{{ __('اسم المورث (صاحب التركة)') }}</label>
                    <input type="text" id="inheritance_from" class="form-control" placeholder="{{ __('الاسم الكامل للمورث الأصلي') }}">
                </div>
                <div class="form-group full">
                    <label class="form-label" for="inheritance_relation">{{ __('صلة قرابة البائع بالمورث') }}</label>
                    <input type="text" id="inheritance_relation" class="form-control" placeholder="{{ __('مثال: والدته المرحومة، والده المرحوم...') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label class="form-label" for="buyer_job">{{ __('الوظيفة / المهنة') }}</label>
                    <input type="text" id="buyer_job" class="form-control" placeholder="{{ __('مثال: موظف حكومي') }}">
                </div>
                <div class="form-group">
        ...
        <div class="form-card">
            <div class="form-card-title">
                <div class="title-with-icon"><i class="fas fa-chart-pie"></i> {{ __('بيانات الحصة الشائعة في المنزل') }}</div>
            </div>
            <div class="form-grid">
                <div class="form-group full">
                    <label class="form-label" for="property_area">{{ __('الناحية / المنطقة') }}</label>
                    <input type="text" id="property_area" class="form-control" placeholder="{{ __('مثال: كوم أمبو - محافظة أسوان') }}">
                </div>
                <div class="form-group full">
                    <label class="form-label" for="property_address">{{ __('رقم المنزل والشارع') }}</label>
                    <input type="text" id="property_address" class="form-control" placeholder="{{ __('مثال: منزل ملك رقم 4 شارع الاتحاد الاشتراكي') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="property_size">{{ __('المساحة الكلية (م²)') }}</label>
                    <input type="text" id="property_size" class="form-control" placeholder="{{ __('مثال: 97.27') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="other_owners"><i class="fas fa-users" style="color: var(--text-secondary); margin-left:4px;"></i> {{ __('باقي الملاك على الشيوع') }}</label>
                    <input type="text" id="other_owners" class="form-control" placeholder="{{ __('مثال: ورثة المرحومة / فوزية عكاشة') }}">
                </div>
                
                {{-- الحدود --}}
                <div class="form-group">
                    <label class="form-label" for="border_north">{{ __('الحد البحري (الشمالي)') }}</label>
                    <input type="text" id="border_north" class="form-control" placeholder="{{ __('يحدها من الشمال...') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="border_south">{{ __('الحد القبلي (الجنوبي)') }}</label>
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="price_number" class="form-control" placeholder="{{ __('مثال: 150,000') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="price_text">{{ __('ثمن البيع (كتابة)') }}</label>
                    <input type="text" id="price_text" class="form-control" placeholder="{{ __('مثال: مائة وخمسون ألف ريالاً عمانياً') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="penalty_number">{{ __('مبلغ التعويض عند الإخلال (أرقام)') }}</label>
                    <input type="text" id="penalty_number" class="form-control" value="100000">
                </div>
                <div class="form-group">
                    <label class="form-label" for="penalty_text">{{ __('مبلغ التعويض (كتابة)') }}</label>
                    <input type="text" id="penalty_text" class="form-control" value="{{ __('مائة ألف ريالاً لاغير') }}">
                </div>
            </div>
        </div>
    ...
    <div class="print-area" id="printArea">
        <h2>{{ __('عقد بيع حصة شائعة في منزل') }}</h2>

        <p>{{ __('إنه في يوم') }} <span id="p_day"></span> {{ __('الموافق') }} <span id="p_date"></span>{{ __('م حرر هذا العقد فيما بين كل من:') }}</p>
        ...
        <p><strong>{{ __('ثانياً: السيد/') }}</strong> <span id="p_buyer_name"></span> - <span id="p_buyer_job"></span> {{ __('- المقيم/') }} <span id="p_buyer_address"></span>{{ __('، ويحمل بطاقة رقم قومي (') }}<span id="p_buyer_id"></span>)</p>
        ...
        <p><strong>{{ __('تمهيد:') }}</strong> {{ __('يمتلك الطرف الأول حصة شائعة في منزل بناحية') }} <span id="p_property_area"></span>{{ __('، وهو عبارة عن /') }} <span id="p_property_address"></span> {{ __('بمساحة كلية') }} <span id="p_property_size"></span>{{ __('م²، وحدوده كالآتي: الحد الشرقي:') }} <span id="p_border_east"></span>{{ __('، والحد الغربي:') }} <span id="p_border_west"></span>{{ __('، والحد البحري:') }} <span id="p_border_north"></span>{{ __('، والحد القبلي:') }} <span id="p_border_south"></span>{{ __('. وقد آلت الملكية بالميراث الشرعي عن') }} <span id="p_inheritance_relation"></span>/ <span id="p_inheritance_from"></span>{{ __('، وحيث أن الطرف الأول عرض هذه الحصة للبيع وتقدم الطرف الثاني لشرائها واتفقا على ما يلي:') }}</p>

        <p><strong>{{ __('البند الأول:') }}</strong> {{ __('يعتبر التمهيد السابق جزءاً لا يتجزأ من هذا العقد.') }}</p>

        <p><strong>{{ __('البند الثاني:') }}</strong> {{ __('باع الطرف الأول وأسقط وتنازل بموجب هذا العقد وبكافة الضمانات الفعلية والقانونية ما هو عبارة عن حصة شائعة في منزل بناحية') }} <span id="p_property_area2"></span>{{ __('، وهو عبارة عن /') }} <span id="p_property_address2"></span> {{ __('بمساحة كلية') }} <span id="p_property_size2"></span>{{ __('م².') }}</p>

        <p><strong>{{ __('البند الثالث:') }}</strong> {{ __('تم هذا البيع بمبلغ إجمالي وقدره (') }}<span id="p_price_number"></span>{{ __('ج) (فقط') }} <span id="p_price_text"></span>{{ __(') دفعه الطرف الثاني عداً ونقداً بأكمله للطرف الأول في مجلس هذا العقد، ويعتبر توقيع الطرف الأول على هذا العقد بمثابة مخالصة بدفع كامل الثمن.') }}</p>
        ...
        <p><strong>{{ __('البند الخامس:') }}</strong> {{ __('يقر الطرف الأول بخلو المبيع من كافة الحقوق العينية الأصلية والتبعية ويقر بأنه لم يتصرف قبل تاريخ هذا العقد في الحصة المبيعة أو في أي جزء منها وأنه يضمن للطرف الثاني عدم التعرض القانوني الصادر منه أو من الغير.') }}</p>

        <p><strong>{{ __('البند السادس:') }}</strong> {{ __('يقر الطرف الأول بأن باقي الملاك على الشيوع') }} <span id="p_other_owners"></span>.</p>
        ...
        <p><strong>{{ __('البند الثامن:') }}</strong> {{ __('يقر الطرفان بأن العنوان الوارد بهذا العقد هو عنوان محل الإقامة لكل منهما وهو المعول عليه فيما يتعلق بالإعلانات والإخطارات التي قد يتطلبها تنفيذ هذا العقد.') }}</p>

        <p><strong>{{ __('البند التاسع:') }}</strong> {{ __('إذا أخل أي طرف من أطراف هذا العقد بأي التزام من الالتزامات المفروضة عليه ببنود هذا العقد يلتزم بدفع تعويض وقدره (') }}<span id="p_penalty_number"></span>{{ __('ج) (فقط') }} <span id="p_penalty_text"></span>{{ __(') للطرف الآخر ولا يخضع هذا التعويض لتقدير القضاء فضلاً عن صحة هذا العقد ونفاذه.') }}</p>
        ...
        <p><strong>{{ __('البند الحادي عشر:') }}</strong> {{ __('تختص محكمة أسوان الجزئية بالفصل في أي نزاع ينشأ بشأن تنفيذ أو تفسير أو صحة ونفاذ هذا العقد.') }}</p>
        ...
        <div class="signature-section">
            <div class="signature-box">
                ...
                <p style="text-align: right;">{{ __('الرقم القومي/') }} <span id="p_seller_id2"></span></p>
