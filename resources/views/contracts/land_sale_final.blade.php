...
<div class="create-page-container">
    ...
    <div class="dashboard-header no-print">
        <div class="header-info">
            <h1><i class="fas fa-map-marked-alt" style="color: var(--gold-accent); margin-left: 8px;"></i> {{ __('عقد بيع نهائي لقطعة أرض') }}</h1>
            <p>{{ __('أدخل بيانات أطراف العقد وتفاصيل قطعة الأرض وحدودها، ثم اضغط على زر المعاينة والطباعة أدناه.') }}</p>
        </div>
    ...
    <div class="no-print">
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label class="form-label" for="seller_religion">{{ __('ديانة البائع') }}</label>
                    <input type="text" id="seller_religion" class="form-control" value="{{ __('مسلم') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="seller_nationality">{{ __('جنسية البائع') }}</label>
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label class="form-label" for="buyer_religion">{{ __('ديانة المشتري') }}</label>
                    <input type="text" id="buyer_religion" class="form-control" value="{{ __('مسلم') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="buyer_nationality">{{ __('جنسية المشتري') }}</label>
        ...
        <div class="form-card">
            <div class="form-card-title">
                <div class="title-with-icon"><i class="fas fa-draw-polygon"></i> {{ __('بيانات قطعة الأرض وحدودها') }}</div>
            </div>
            <div class="form-grid">
                <div class="form-group full">
                    <label class="form-label" for="property_location">{{ __('موقع قطعة الأرض') }}</label>
                    <input type="text" id="property_location" class="form-control" placeholder="{{ __('مثال: أسوان - حي ...') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="property_street">{{ __('الشارع / المنطقة') }}</label>
                    <input type="text" id="property_street" class="form-control" placeholder="{{ __('مثال: شارع المحطة') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="property_area">{{ __('المساحة') }}</label>
                    <input type="text" id="property_area" class="form-control" placeholder="{{ __('مثال: 500 متر مربع') }}">
                </div>
                
                {{-- الحدود --}}
                <div class="form-group">
                    <label class="form-label" for="border_north">{{ __('الحد البحري (شمالاً)') }}</label>
                    <input type="text" id="border_north" class="form-control" placeholder="{{ __('يحدها من الشمال...') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="border_south">{{ __('الحد القبلي (جنوباً)') }}</label>
                    <input type="text" id="border_south" class="form-control" placeholder="{{ __('يحدها من الجنوب...') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="border_east">{{ __('الحد الشرقي') }}</label>
                    <input type="text" id="border_east" class="form-control" placeholder="{{ __('يحدها من الشرق...') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="border_west">{{ __('الحد الغربي') }}</label>
                    <input type="text" id="border_west" class="form-control" placeholder="{{ __('يحدها من الغرب...') }}">
                </div>

                <div class="form-group full">
                    <label class="form-label" for="property_origin">{{ __('كيف آلت الملكية للبائع') }}</label>
                    <input type="text" id="property_origin" class="form-control" placeholder="{{ __('مثال: عن طريق الشراء بموجب العقد المؤرخ في ...') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="price_number" class="form-control" placeholder="{{ __('مثال: 2,000,000') }}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="price_text">{{ __('ثمن البيع (كتابة)') }}</label>
                    <input type="text" id="price_text" class="form-control" placeholder="{{ __('مثال: اثنان مليون ريالاً عمانياً') }}">
                </div>
                <div class="form-group">
    ...
    <div class="print-area" id="printArea">
        <h2>{{ __('عقد بيع نهائي لقطعة أرض') }}</h2>

        <p>{{ __('إنه في يوم الموافق') }} <span id="p_date"></span> {{ __('بين كلٍ من:') }}</p>

        <p><strong>{{ __('أولاً: السيد/') }}</strong> <span id="p_seller_name"></span> - <span id="p_seller_nationality"></span> - <span id="p_seller_religion"></span> {{ __('- بالغ سن الرشد - المقيم بـ') }} <span id="p_seller_address"></span> {{ __('- ويحمل بطاقة رقم قومي (') }}<span id="p_seller_id"></span>)</p>
        <p style="text-align: left; font-weight: bold; margin-bottom: 1.5rem;">{{ __('(طرف أول بائع)') }}</p>

        <p><strong>{{ __('ثانياً: السيد/') }}</strong> <span id="p_buyer_name"></span> - <span id="p_buyer_nationality"></span> - <span id="p_buyer_religion"></span> {{ __('- بالغ سن الرشد - المقيم بـ') }} <span id="p_buyer_address"></span> {{ __('- ويحمل بطاقة رقم قومي (') }}<span id="p_buyer_id"></span>)</p>
        <p style="text-align: left; font-weight: bold; margin-bottom: 1.5rem;">{{ __('(طرف ثاني مشتري)') }}</p>

        <p>{{ __('بعد أن أقر الطرفان بأهليتهما للتصرف والتعاقد والتراضي اتفقا فيما بينهما على ما يلي:') }}</p>

        <p><strong>{{ __('البند التمهيدي:') }}</strong> {{ __('يمتلك الطرف الأول (البائع) قطعة أرض مساحة كائنة بـ') }} <span id="p_property_location"></span> {{ __('- شارع') }} <span id="p_property_street"></span> {{ __('تبلغ مساحتها (') }}<span id="p_property_area"></span>{{ __(') تحت العجز والزيادة، وحدودها كالتالي: الحد البحري:') }} <span id="p_border_north"></span>{{ __('، الحد القبلي:') }} <span id="p_border_south"></span>{{ __('، الحد الشرقي:') }} <span id="p_border_east"></span>{{ __('، الحد الغربي:') }} <span id="p_border_west"></span>{{ __('. وقد آلت الملكية إليه') }} <span id="p_property_origin"></span>.</p>

        <p><strong>{{ __('البند الأول:') }}</strong> {{ __('يعتبر التمهيد السابق جزءاً لا يتجزأ من هذا العقد مكملاً ومفسراً لبنوده.') }}</p>
        
        <p><strong>{{ __('البند الثاني:') }}</strong> {{ __('باع وأسقط وتنازل بكافة الضمانات القانونية والفعلية الطرف الأول إلى الطرف الثاني قطعة الأرض الكائنة بـ') }} <span id="p_property_location2"></span> {{ __('- شارع') }} <span id="p_property_street2"></span> {{ __('تبلغ مساحتها (') }}<span id="p_property_area2"></span>{{ __(') تحت العجز والزيادة.') }}</p>
        
        <p><strong>{{ __('البند الثالث:') }}</strong> {{ __('يقر الطرف الثاني (المشتري) بأنه عاين قطعة الأرض المعاينة التامة النافية للجهالة وقبل شراءها بحالتها التي هي عليها وقت التعاقد.') }}</p>
        
        <p><strong>{{ __('البند الرابع:') }}</strong> {{ __('تم هذا البيع نظير ثمن إجمالي وقدره (') }}<span id="p_price_number"></span>{{ __(') (فقط') }} <span id="p_price_text"></span> {{ __('لاغير) دفعها الطرف الثاني عداً ونقداً بمجلس هذا العقد ويعتبر توقيعه على هذا العقد بمثابة مخالصة نهائية بقبض كامل الثمن.') }}</p>
        
        <p><strong>{{ __('البند الخامس:') }}</strong> {{ __('يقر الطرف الأول (البائع) بأن الأرض المبيعة موضوع عقد البيع في حيازته وأن هذه الحيازة هادئة ومستقرة وأنه هو الحائز الوحيد للأرض المبيعة.') }}</p>
        
        <p><strong>{{ __('البند السادس:') }}</strong> {{ __('بموجب هذا العقد وبمجرد التوقيع عليه يلتزم الطرف الأول بتسليم الطرف الثاني الأرض موضوع البيع، ويحق لهذا الأخير وضع يده على هذه الأرض دون أدنى معارضة من الطرف الأول، وفي حالة وجود أي حائز آخر يكون الطرف الأول ملزماً برد ثمن البيع مضافاً إليه مبلغ (') }}<span id="p_penalty_number"></span>{{ __(') كشرط جزائي.') }}</p>
        
        <p><strong>{{ __('البند السابع:') }}</strong> {{ __('يلتزم الطرف الأول بالحضور مع الطرف الثاني أمام المحكمة المختصة للإقرار بصحة توقيعه على هذا العقد أو إعطاء توكيل له في ذلك الشأن، كما يلتزم بالحضور أمام جميع الجهات الحكومية أو غير الحكومية لاتخاذ إجراءات نقل الملكية للطرف الثاني متى طلب منه ذلك.') }}</p>
        
        <p><strong>{{ __('البند الثامن:') }}</strong> {{ __('يقر الطرف الأول (البائع) بخلو الأرض المبيعة موضوع هذا العقد من كافة الرهون أو الديون أو الحقوق العينية للغير.') }}</p>
        
        <p><strong>{{ __('البند التاسع:') }}</strong> {{ __('يلتزم الطرفان في حالة الفسخ أو الإخلال بأي بند من بنود هذا العقد بدفع مبلغ وقدره (') }}<span id="p_penalty_breach"></span>{{ __(') للطرف الآخر كشرط جزائي في العقد.') }}</p>
        
        <p><strong>{{ __('البند العاشر:') }}</strong> {{ __('تختص') }} <span id="p_court_name"></span> {{ __('على اختلاف درجاتها دون غيرها بكافة النزاعات التي تنشأ عن تفسير أو توثيق هذا العقد.') }}</p>
        
        <p><strong>{{ __('البند الحادي عشر:') }}</strong> {{ __('حرر هذا العقد من نسختين بيد كل طرف نسخة للعمل بموجبها عند اللزوم.') }}</p>
