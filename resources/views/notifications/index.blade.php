@extends('layouts.app')
@section('title', 'الإشعارات | ' . $appName)
...
<style>
    ...
    /* ===== Mark-read form (inline, no button shown — whole row is clickable) ===== */
    .notif-mark-form { display: none; }
    ...
    @else
        <div class="notif-list">
            @foreach($notifications as $notification)
                ...
                {{-- Clicking the whole card marks as read and redirects --}}
                <form action="{{ route('notifications.read', $notification->id) }}"
                      method="POST" style="margin:0;">
                    @csrf
                    <button type="submit"
                            class="notif-item {{ $isUnread ? 'unread' : 'read' }}"
                            style="width:100%; text-align:right; background:inherit; font-family:'Tajawal',sans-serif;">
